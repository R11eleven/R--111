...
## narrator ####################################################################
# 在 screens.rpy 中修改 screen_text 屏幕
screen screen_text(text, duration=3.0):
    zorder 100
    modal False
    ...
    frame:
        ...
        text text:
            ...
            text_align 0.5
            xalign 0.5
            slow_cps 20  # 控制打字速度，数字越大越快
    
    # 计算持续时间（根据文字长度）
    $ text_duration = len(text) * 0.1 + 1.0  # 每个字符0.1秒 + 额外1秒
    timer text_duration action Hide("screen_text")
   ...
   # 标题
    ...
    vbox:
        ...
        button:
            ...
            ysize 100
            action [
                Function(init_new_game),  # 初始化新游戏
                ShowMenu("chapter_select")  # 进入章节选择
            ]
            ...
            vbox:
                ...
                text "开始新游戏":
                    style "mode_button_title"
                    xalign 0.5
                
                text "从头体验完整故事":
                    style "mode_button_desc"
                    xalign 0.5
        ...
        if renpy.list_saved_games(fast=True):
            button:
                ...
                ysize 100
                action [
                    Function(load_understanding_data),  # 加载已有进度
                    ShowMenu("load")  # 进入读档界面
                ]
        ...
        # 4. 特别模式（可选）
        if persistent.all_chapters_completed:  # 通关后解锁
            button:
                style "mode_button"
                xsize 400
                ysize 100
                action Start("special_mode")
...
screen chapter_select():
    ...
    use game_menu(_("章节选择"), scroll="viewport"):
        ...
        vbox:
            ...
            python:
                chapters = [
                    {"id": "chapter_1", "name": "第一章", "desc": "开始你的冒险"},
                    {"id": "chapter_2", "name": "第二章", "desc": "冒险继续"},
                    {"id": "chapter_3", "name": "第三章", "desc": "深入探索"},
                    {"id": "chapter_4", "name": "第四章", "desc": "揭开秘密"},
...
screen character_archive():
    tag menu
    
    use game_menu(_("人物总览"), scroll="viewport"):
        ...
        vbox:
            ...
            python:
                character_groups = [
                    ("神明", ["Crate", "Morlice", "生命之母", "死亡女神"]),
                    ("天堂领主", ["Justice","Mood","孟双朔","Viridis","乌重九","Nyx","贞洁"]),
                    ("地狱领主", ["Lucifer","Jealous","Fury","Schorl","Voracity","Gluttony","Lust"]),
                    ("一代成员", ["Dust", "X", "Sakura", "四夕", "Ryusui", "Glitter", "Talent"]),
                    ("二代成员", ["Mithar", "Monial", "皇甫子伥", "Corrian", "Xenon", "Alpine", "祁溟", "Luna", "Aria", "Regsya", "Amily"]),
                    ("其他", ["Kaward", "Glant", "Gloria", "Wynn"])
                ]
            ...
            for group_name, chars in character_groups:
                ...
                hbox:
                    ...
                    for char in chars:
                        $ is_unlocked = is_character_unlocked(char)
                        $ display_name = get_character_display_name(char)
                        $ level_name, level_color, level_icon = get_level_info(char)
                        ...
                        button:
                            if is_unlocked:
                                ...
                                insensitive_background "#cacaca"
                                padding (20, 20)
                                xsize 200
                                ysize 120
                            
                            # 点击事件：已解锁则跳转到详情，未解锁则不响应
                            if is_unlocked:
                                action Show("character_detail", character=char)
                            else:
                                action None
